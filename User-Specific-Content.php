...
function stan_function() {
        ...
        echo 'Sending a command to open the Coworking space roller shutters!';
}
function light_off_hook() {
    do_action('light_off_hook');
}

function light_off_function() {
        echo 'Sending a command to turn off office Lights!';
}


function light_on_hook() {
    do_action('light_on_hook');
}

function light_on_function() {
        echo 'Sending a command to turn on office Lights!';
}


		add_action('light_on_hook', 'light_on_function');
...
public function light_off_hook() {
    do_action('light_off_hook');
}

public function light_off_function() {
        echo 'Sending a command to turn off office Lights!';
}


public function light_on_hook() {
    do_action('light_on_hook');
}

public function light_on_function() {
        echo 'Sending a command to turn on office Lights!';
}
	...
	public function User_specific_content_filter($content){
		...
		add_action('light_off_hook', 'light_off_function');
		add_action('light_on_hook', 'light_on_function');
		$savedoptions = get_post_meta($post->ID, 'U_S_C_options',true);
		...
		if (isset($savedusers) && !empty($savedusers)){
			...
			if($joro === 461){
                        	...
                        	light_off_hook();
                     //  	wp_redirect( 'https://hwgroup-bg.com' );
}
 			if($joro === 498){
